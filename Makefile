...
.PHONY: all iso image boot bootloader kernel run run-disk clean FORCE
...
$(DISK_IMAGE): $(BOOTSTRAP_IMAGE) $(STAGE2_IMAGE) $(KERNEL_IMAGE) $(BOOT_CONFIG) FORCE
	...
	$(MCOPY) -i $(FAT_IMAGE) $(STAGE2_IMAGE) ::STAGE2.BIN
	$(MCOPY) -i $(FAT_IMAGE) $(KERNEL_IMAGE) ::KERNEL.ELF
	$(MCOPY) -i $(FAT_IMAGE) $(BOOT_CONFIG) ::AVBOOT.CONF
...
run run-disk: $(DISK_IMAGE)
	$(QEMU) -drive file=$<,format=raw -m 2G -serial stdio -vga std
