...
void SerialInit(uint16_t Port, uint32_t Baud)
{
	...
	PortIOWrite(PortIOWidth8, SerialInterrupt(Port), 0x0F);
}

static bool SerialIsTransmitEmpty(void)
{
	return (PortIORead(PortIOWidth8, SerialLineStatus(SerialPort)) & 0x20) != 0;
}
...
void SerialPutc(char Character)
{
	...
	if (Character == '\n') {
		PortIOWrite(PortIOWidth8, SerialData(SerialPort), '\r');
		while (!SerialIsTransmitEmpty()) {
			__asm__ volatile("pause");
		}
	}

	PortIOWrite(PortIOWidth8, SerialData(SerialPort), (uint8_t)Character);
}
